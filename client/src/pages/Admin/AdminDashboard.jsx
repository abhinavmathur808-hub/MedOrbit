...
const AdminDashboard = () => {
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            ...
            default: return <span className="px-2 py-1 bg-neutral-800 text-neutral-400 rounded-full text-xs font-medium">{status}</span>;
        }
    };
        ...
        return (
            <div className="min-h-screen bg-neutral-950">
                <div className="min-h-[calc(100vh-64px)] flex items-center justify-center">
                    <Loader2 className="w-12 h-12 text-burgundy-500 animate-spin" />
                </div>
            </div>
        );
    }
            ...
            <CurvedWrapper>
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                        <div className="rounded-2xl p-6 hover:-translate-y-0.5 transition-all duration-200" style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', boxShadow: 'var(--card-shadow)' }}>
                            <div className="flex items-center gap-4">
                                <div className="w-14 h-14 bg-burgundy-600/10 rounded-xl flex items-center justify-center">
                                    <Users className="w-7 h-7 text-burgundy-400" />
                                </div>
                                <div>
                        ...
                        <div className="rounded-2xl p-6 hover:-translate-y-0.5 transition-all duration-200" style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', boxShadow: 'var(--card-shadow)' }}>
                            <div className="flex items-center gap-4">
                                <div className="w-14 h-14 bg-burgundy-600/10 rounded-xl flex items-center justify-center">
                                    <Stethoscope className="w-7 h-7 text-burgundy-400" />
                                </div>
                                <div>
                                    <p className="text-3xl font-bold text-white">{stats.totalDoctors}</p>
                                    <p className="text-neutral-500 text-sm">Total Doctors</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="rounded-2xl p-6 hover:-translate-y-0.5 transition-all duration-200" style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', boxShadow: 'var(--card-shadow)' }}>
                            <div className="flex items-center gap-4">
                                <div className="w-14 h-14 bg-burgundy-500/10 rounded-xl flex items-center justify-center">
                                    <Calendar className="w-7 h-7 text-burgundy-400" />
                                </div>
                                <div>
                    ...
                    <div className="grid lg:grid-cols-3 gap-6 mb-8">
                        <div className="lg:col-span-2 rounded-2xl p-6" style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', boxShadow: 'var(--card-shadow)' }}>
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                                    <Activity className="w-5 h-5 text-burgundy-500" />
                                    Recent Activity
                                </h2>
                                <span className="text-sm text-neutral-600">Last 5 bookings</span>
                            </div>
                        ...
                        <div className="rounded-2xl p-6" style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', boxShadow: 'var(--card-shadow)' }}>
                            <h2 className="text-lg font-bold text-white flex items-center gap-2 mb-6">
                                <TrendingUp className="w-5 h-5 text-burgundy-400" />
                                Doctor Status
                            </h2>
                    ...
                    <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', boxShadow: 'var(--card-shadow)' }}>
                            ...
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                ...
                                <div className="relative max-w-xs w-full">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-10 pr-4 py-2 bg-black/50 border border-neutral-800 rounded-lg focus:border-neutral-500 focus:ring-1 focus:ring-neutral-500 outline-none text-sm text-white placeholder-neutral-600"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead className="bg-white/[0.03] border-b border-neutral-800">
                                    <tr>
                                        <th className="text-left py-4 px-6 font-semibold text-neutral-400">Doctor</th>
                                        <th className="text-left py-4 px-6 font-semibold text-neutral-400">Email</th>
                                        <th className="text-left py-4 px-6 font-semibold text-neutral-400">Joined</th>
                                        <th className="text-left py-4 px-6 font-semibold text-neutral-400">Status</th>
                                        <th className="text-center py-4 px-6 font-semibold text-neutral-400">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {filteredDoctors.length === 0 ? (
                                        <tr>
                                            <td colSpan="5" className="py-12 text-center text-neutral-600">
                                                <Users className="w-10 h-10 mx-auto mb-2 text-neutral-700" />
                                                No doctors found
                                            </td>
                                        </tr>
                                    ) : (
                                        filteredDoctors.map((doctor) => (
                                            <tr key={doctor._id} className="border-b border-neutral-800/50 hover:bg-white/[0.03]">
                                                <td className="py-4 px-6">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-10 h-10 bg-gradient-to-br from-burgundy-600 to-burgundy-800 rounded-full flex items-center justify-center text-white font-semibold">
                                                            {doctor.photo ? (
                                                        ...
                                                        <div className="flex items-center gap-1">
                                                            ...
                                                            {doctor.isVerified && <BadgeCheck className="w-4 h-4 text-burgundy-400" />}
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="py-4 px-6 text-neutral-400 text-sm">{doctor.email}</td>
                                                ...
                                                <td className="py-4 px-6 text-center">
                                                    {doctor.isVerified ? (
                                                        ...
                                                        <button
                                                            ...
                                                            className="inline-flex items-center gap-1 px-3 py-1.5 bg-burgundy-600 hover:bg-burgundy-500 text-white rounded-lg text-sm font-medium disabled:opacity-50"
                                                        >
