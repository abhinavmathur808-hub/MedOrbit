            ...
            <section className="relative w-full min-h-[85vh] flex items-center justify-center overflow-hidden pb-20 pt-32">
                ...
                <div className="relative z-20 w-full max-w-6xl mx-auto flex flex-col items-center text-center px-4 md:px-8">
                    ...
                    <h1 className="text-5xl sm:text-6xl lg:text-7xl font-semibold tracking-tight leading-none text-white mb-6">
                        Bring <span className="relative inline-block border border-burgundy-700/50 bg-burgundy-900/20 text-neutral-200 px-3 py-1 mx-2">
                            <span className="absolute -top-[3px] -left-[3px] w-1.5 h-1.5 bg-burgundy-600 border border-burgundy-700/50"></span>
                            <span className="absolute -top-[3px] -right-[3px] w-1.5 h-1.5 bg-burgundy-600 border border-burgundy-700/50"></span>
                            <span className="absolute -bottom-[3px] -left-[3px] w-1.5 h-1.5 bg-burgundy-600 border border-burgundy-700/50"></span>
                            <span className="absolute -bottom-[3px] -right-[3px] w-1.5 h-1.5 bg-burgundy-600 border border-burgundy-700/50"></span>
                            Doctors
                        </span><br />
                        Into Your Orbit!
                    </h1>

                    <p className="text-lg sm:text-xl leading-relaxed text-neutral-300 max-w-xl mx-auto mb-14">
                        Find the right doctor and book your appointment instantly. Scheduling made
                        simple, reliable and fast.
                    </p>

                    <div
                        className="w-full max-w-2xl mx-auto rounded-2xl p-6 border border-neutral-800"
                        style={{
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <div className="flex-1 flex items-center bg-black/50 rounded-xl p-4 border border-neutral-800 focus-within:border-neutral-500 transition-colors duration-200">
                                <Search className="w-6 h-6 text-neutral-600 mr-3 flex-shrink-0" />
                                <input
                                    type="text"
                                    value={searchQuery}
                                    ...
                                    className="w-full outline-none text-lg bg-transparent text-white placeholder-neutral-600"
                                />
                                {searchQuery && (
                                    <button
                                        onClick={clearSearch}
                                        className="text-neutral-600 hover:text-neutral-300 transition-colors flex-shrink-0 ml-2"
                                        aria-label="Clear"
                                    >
                                        <X className="w-5 h-5" />
                                    </button>
                            ...
                            <button
                                ...
                                className="bg-burgundy-600 text-white rounded-full px-8 py-4 font-medium text-lg hover:bg-burgundy-500 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 flex-shrink-0 cursor-pointer shadow-lg shadow-burgundy-900/40"
                            >
                                Search
                            </button>
                        </div>
                        ...
                        {triageStatus === 'loading' && (
                            <p className="text-[11px] text-burgundy-400/80 mt-3 text-left flex items-center gap-1.5 pl-1">
                                <Loader2 className="w-3 h-3 animate-spin" />
                                Preparing on-device AI… {modelProgress}% (one-time, then instant)
                            </p>
                        )}
                        {triageStatus === 'ready' && !hasSearched && (
                            <p className="text-[11px] text-neutral-500 mt-3 text-left flex items-center gap-1.5 pl-1">
                            ...
                            {hasSearched && (
                                ...
                                >
                                    <div className="mt-4 pt-4 border-t border-white/10 text-left">
                                        ...
                                        <div className="flex items-center gap-1.5 text-xs font-semibold text-burgundy-400 mb-2">
                                        ...
                                        {!triage ? (
                                            <div className="flex items-center gap-2 text-neutral-400 text-sm">
                                                <Loader2 className="w-4 h-4 animate-spin text-burgundy-400" />
                                                Analyzing your symptoms…
                                            </div>
                                        ) : lowConfidence ? (
                                            <p className="text-neutral-300 text-sm">
                                                I couldn't pin that down confidently — starting you with a{' '}
                                                <strong className="font-semibold text-burgundy-400">General Physician</strong>{' '}
                                                is a safe bet. Add a little more detail for a sharper match.
                                            </p>
                                        ) : (
                                            <p className="text-neutral-300 text-sm">
                                                Your symptoms point to a{' '}
                                                <strong className="font-semibold text-burgundy-400">{matchedSpecialty}</strong>
                                                {triage.confidence === 'moderate' ? ' (possible match)' : ''}.
                                                {refining && (
                                                    <span className="text-neutral-500 inline-flex items-center gap-1 ml-1">
                                                        <Loader2 className="w-3 h-3 animate-spin" />
                                                        refining…
                                                    </span>
                                                )}
                                        ...
                                        {doctorsLoading && (
                                            <div className="flex items-center gap-2 text-neutral-500 text-xs mt-3">
                                            ...
                                            <div className="mt-3 grid sm:grid-cols-3 gap-2">
                                                {doctors.map((doc) => (
                                                    <Link
                                                        ...
                                                        className="group flex flex-col gap-1 px-3 py-2.5 bg-white/[0.03] hover:bg-white/[0.06] border border-white/10 hover:border-burgundy-500/40 rounded-xl transition-all"
                                                    >
                                                        <span className="flex items-center gap-1 font-medium text-neutral-100 text-sm truncate">
                                                            {doc.userId?.name || 'Doctor'}
                                                            {doc.userId?.isVerified && <BadgeCheck className="w-3.5 h-3.5 text-blue-400 flex-shrink-0" />}
                                                        </span>
                                                        <span className="flex items-center justify-between text-xs">
                                                            <span className="text-neutral-500">₹{doc.fees}</span>
                                                            <span className="text-burgundy-400 font-semibold flex items-center gap-0.5 group-hover:gap-1.5 transition-all">
                                        ...
                                        {triage && !lowConfidence && (
                                            <button
                                                ...
                                                className="mt-3 text-xs text-neutral-400 hover:text-burgundy-400 transition-colors inline-flex items-center gap-1"
                                            >
                                        ...
                                        {(adviceLoading || advice || adviceError || !user) && (
                                            <div className="mt-4 pt-4 border-t border-white/10">
                                                <div className="flex items-center gap-1.5 text-xs font-semibold text-burgundy-300 mb-2">
                                                ...
                                                {!user ? (
                                                    <p className="text-neutral-400 text-sm">
                                                        <Link to="/login" className="text-burgundy-400 font-medium hover:underline">Log in</Link>{' '}
                                                        for detailed AI guidance. The instant match above works without an account.
                                                    </p>
                                                ) : adviceLoading ? (
                                                    <div className="flex items-center gap-2 text-neutral-400 text-sm">
                                                        <Loader2 className="w-4 h-4 animate-spin text-burgundy-300" />
                                                        Generating detailed guidance…
                                                    </div>
                                                ) : adviceError ? (
                                                    <p className="text-neutral-500 text-sm">{adviceError}</p>
                                                ) : (
                                                    <div className="text-neutral-300 text-sm leading-relaxed">
                                                        <ReactMarkdown
                                                            components={{
                                                                strong: ({ children }) => <strong className="font-semibold text-burgundy-400">{children}</strong>,
                                        ...
                                        {/* Privacy + disclaimer */}
                                        <div className="flex items-center gap-1 text-[10px] text-neutral-600 mt-4">
                    ...
                    {healthQuote && !hasSearched && (
                        <p className="mt-10 text-sm sm:text-base w-full leading-relaxed text-center text-neutral-200">
                            <span className="drop-shadow-[0_0_12px_rgba(253,224,71,0.9)] text-lg inline-block mr-2">💡</span>{healthQuote}
                        </p>
                    )}
                </div>

                <div className="absolute bottom-0 left-0 w-full h-48 bg-gradient-to-t from-[#0a0a0a] to-transparent z-10 pointer-events-none"></div>
            </section>

            <div className="relative z-10 bg-neutral-950 pt-16 pb-24 px-6 md:px-12 w-full">
